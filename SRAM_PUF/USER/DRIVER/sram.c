...
void SPI_SRAM_PUF_FILE(unsigned char* Infos,int len){
		...
		//信息解析
		for(int i = 0 ; i<16;i++){
			User.userLabel[i/4] = *(Infos+i);
			File.fileLabel[i/4] = *(Infos+i+4);
		}
		...
		for(;startPoint<maxUser;startPoint+=20){
			uint32_t tempUser[4];
		...
		if(flag !=4){ //没找到用户，创建用户，并保存文件
			...
			SPI_SRAM_PUF_FILE_LABEL(File.fileLabel,sizeof(File.fileLabel),File.helpData); //生成helpdata，输出密钥
			File.nextAdd = 0;
			...
			if(tmpFile.fileLabel == File.fileLabel){
					uint32_t sramData[32];
					uint32_t xorData[32];
					uint32_t codeData[32];
					uint32_t Data[32];
